# BPM-Caddy — architecture and design notes
...
The working summary for day-to-day changes is `CLAUDE.md` at the repo
root; this file keeps the full reasoning, and several of its sentences
are read verbatim by tests in `src/strings.rs`.
...
- root `bpm-caddy` — the app: `src/app.rs` (UI/state), `src/db.rs`
  ...
  **« no data » is not « no risk »** (`SansDonnee` is a level apart from
  ...
  since `review` likewise runs the whole table. Its `Without` shape
  carries a **veto** as well, and for the reason `renal` and `gravidity`
  carry theirs: a line the veto catches is not *named* by the trigger
  group, which is what lets it supply what is missing instead. « A
  methotrexate with no folic acid » could not be written without it —
  the Lederfoline card says « antidote du méthotrexate » in its class,
...
The one that was missing — « combien de fois un autre poste avait
  ...
  66 of them, and counting at all but one would be a counter quietly
...
- **`spacing.scroll.floating` is egui's default and nothing here changes
  ...
  fold is the control you came to use. 45 regions now set
  ...
  « Enregistrer ». The forty-fifth is a card's history of versions, whose
  « Revenir à cette version » and « Adopter » are the gestures.
...
- `BPM_CADDY_START_VIEW=verrou|search|dashboard|patient|patient_edit|patient_new|drugs|drug_card|agenda|agenda_day|
  ...
  vaccine_map|vaccins_grossesse|vaccins_catalogue|ruptures|reseau|versions|ordonnance|ordonnance_lignes|rein|grossesse|age|cyp|ddi|ddi_crush|libelles|listes|base|codex|
