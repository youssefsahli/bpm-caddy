...
The one that was missing — « combien de fois un autre poste avait
  ...
  62 of them, and counting at all but one would be a counter quietly
...
- **`spacing.scroll.floating` is egui's default and nothing here changes
  ...
  fold is the control you came to use. 38 regions now set
  ...
  the form — and a pregnancy adds five.
...
- `BPM_CADDY_START_VIEW=verrou|search|dashboard|patient|patient_edit|patient_new|drugs|drug_card|agenda|agenda_day|
  ...
  vaccine_map|vaccins_grossesse|ordonnance|ordonnance_lignes|rein|grossesse|age|cyp|ddi|ddi_crush|libelles|listes|base|codex|
