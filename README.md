...
- **Drug reference base** — `F3` opens the team's shared drug cards (DCI, dosage, interactions, IUP, antidote, notes): two typed letters — brand or DCI — show the essentials at a glance, no match becomes a new card, and any card inserts into the team notes in one click. A fresh base starts with ~275 common drugs (names, DCI, classes, textbook antidotes), of which the ~30 the interviews turn around (AOD, AVK, HBPM, inhalers, narrow-margin drugs, oral anticancer) ship with their posology, interactions, patient advice and pharmacokinetics filled in — reference text you can edit, and which never overwrites what the team has written. Stored encrypted with the rest.
