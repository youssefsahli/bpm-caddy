...
- **Global search (`Ctrl+K`)** — « Aller à… » searches patients, drug cards (brand and DCI), reference tables, preparations, medical devices, protocols and views in one list. Arrows to move, `Entrée` to open, `Échap` to close without changing the view. The last row runs a full-text search of the monographs.
...
- **Dashboard** — indicators, pipeline funnel, monthly billed vs. pending revenue, act mix and quotas, 28-day load, upcoming appointments (printable). « À programmer » lists accompaniments with no appointment scheduled; « À revoir » is the call list (biology, ordonnance review, overdue monitoring), printable.
