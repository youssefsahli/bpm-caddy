...
- **Connection map** — the officine's posts and the network's officines drawn around this post, each link in its state (solid, thin, dashed, dotted); click a node to write, send a file, dial it or sync. Every BPM-Caddy on the local network shows up — neighbouring officines on an « À portée » arc, unlinked posts on the posts ring — each officine with a badge (initials in a colour drawn from its fingerprint, identical on every post) and its town.
- **Connection at launch** — paired officines connect by themselves while the application is open: each answers its own paired officines only, a paired officine heard on the local network is dialled at once, and one reachable from outside (router, VPN) announces its address to its network's members. Pairing uses a one-time invitation code (`K7QM-2XPA-9D3F-7H1Q@192.168.1.20:7742`) that both ends prove over the handshake, so nothing is typed but the code and nothing is compared.
