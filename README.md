...
![Recherche dans les monographies — chaque fiche qui dit le mot, avec la phrase qui le porte](docs/screenshot_mono.png)
...
- **One box over everything (`Ctrl+K`)** — « Aller à… » searches the whole base at once and goes there: a patient, a fiche and its DCI, a reference table, a préparation of the codex, a dispositif médical, a substitution protocol, or one of the standing views. The arrows walk the list, `Entrée` opens, `Échap` leaves the view exactly where it was. Its last row is always « chercher ce mot dans le texte des fiches » — what no name matches is often written inside a monograph, and three letters are enough to go and look. Each kind takes at most four of the twelve rows before the others get a turn — with eight hundred fiches in the base, a plain ranking would be eight hundred fiches — and the rest of the box fills from what is left over, so a query that only matches fiches still fills it.
...
- **An A–Z tree over eight hundred fiches** — 850 monographs are not a list anybody scrolls; they are twenty-seven drawers anybody opens. Typed into, the drug navigator is the flat list of best matches; empty, it is the alphabet, each letter carrying its count, the drawer of the fiche you are reading always open. Either way a row reads « Aclasta *acide zolédronique* »: the brand is what is on the box, the DCI is what the question was actually about, and it is the DCI that is elided when the dock is dragged narrow. Accents file with their letter, so « Élugan » sits under E and not in a drawer of its own past Z.
- **The carte Vitale opens the file** — a reader on the counter, and presenting the card is a way of naming a patient: the card's beneficiaries appear under the search field — the holder and the ayants droit, because the person at the counter is not always the holder — and one click opens the file that carries that NIR, or starts a new one already filled with the name and the date of birth. It matches on the **NIR and not on the name**: two people share a name, nobody shares a number. Nothing is guessed from an offset either — the card's bytes are searched for a fifteen-digit run whose own control key checks out, so a file laid out differently by another card generation is read the same way or not at all, and a date beside it becomes a date of birth only if it agrees with the month and the year the NIR itself carries. The system's PC/SC library is opened by name at the moment a card is asked for and never linked against, so a post with no reader — most posts — keeps an application that starts. It reads identity and bills nothing: a feuille de soins électronique needs an agreed SESAM-Vitale package and a carte CPS, and the officine's LGO stays the tool that produces it.
- **The shape the post was left in** — window size, dock widths, which docks were open, what the right pane was showing and which view was on screen come back next morning, recorded by working rather than set in a dialog. Options › À propos states what this copy is, what its base holds, and — on a button press, on a thread of its own, the one network request the application ever makes — whether GitHub has a newer release. Everything else marked « … » hands a URL to the browser and stops there.
- **Keyboard-driven workflow** — `Ctrl+F` search, `Enter` select, `Ctrl+N` new interview. Zero loading screens.
- **The interview sheet carries the entretien** — the printed fiche holds the treatments the file knows and the checklist of the act's own theme, as tick-boxes: what to cover for an initiation, for an observance, for a biology review, for an inhaler technique. A theme the officine wrote itself gets the common ground.
...
- **Drug reference base** — `F3` opens the team's shared drug cards (DCI, dosage, interactions, IUP, antidote, notes): two typed letters — brand or DCI — show the essentials at a glance, no match becomes a new card, and any card inserts into the team notes in one click. A fresh base starts with 850 common drugs, every one of them a full monograph — indications, mechanism, posology, contraindications, interactions, adverse effects, monitoring, counselling points, pharmacokinetics and numbered sources — covering the anticoagulants, the inhalers, the narrow-margin drugs, the oral anticancer drugs, the antibiotics and the psychotropes. On top of the monograph, 1 662 posology lines on 724 of those cards, indication by indication, each with the remark that matters at the counter — and, on 818 of them, what to do when a dose is missed and what must send the patient to a doctor. Every field is editable, and a top-up never overwrites what the team has written. Stored encrypted with the rest.
...
- **The vaccination act and the carnet line, written once** — from the act, one button jumps to the carnet with the day and the initials filled; from the carnet, a dose recorded today with no act says so and offers to create it. A dose given without its acte is work already done and not billed.
...
- **Editable PDF templates** — the Typst sources of the interview sheet, the CR letter, the carnet page and the ordonnance are editable in-app ("Modèles…"), with compile validation, sample previews and the list of markers each one may use; no recompilation needed.
- **Recall-ready** — every drug card lists the patients currently on it, one click from their record.
- **Substitution protocols** — what to dispense when a drug cannot be, written as a decision tree: questions with their oui/non branches, conduites to follow, walked one step at a time at the counter and printable as an A4 page. Five come with the base — la rupture de stock, l'AOD indisponible, l'écrasement demandé, l'allergie à la pénicilline annoncée au comptoir, la fièvre chez un patient à risque — and a tree the team rewrites is never replaced.
- **Reference tables** — thirty-four counter references browsable in-app and printable as a four-page A4 sheet, each with its numbered sources: dose equivalences (IPP, statines, corticoïdes, opioïdes, benzodiazépines), dosing references (HBPM, AOD with renal adaptation, inhaled corticosteroid steps, insulin profiles, non-opioid analgesics), and the decision aids the acts need (Cockcroft and CKD stages, Mac Isaac score for the angina TROD, first-line cystitis treatments, missed-pill conduct, adult vaccination boosters, paediatric doses by weight, and what may or may not be crushed), the three that answer a question asked without an ordonnance in hand (interactions to catch at the counter, the emergencies to recognise and orient, what may be dispensed in pregnancy and while breastfeeding), and seven that answer the questions the others did not: the médicaments to reassess after 75 and what is proposed instead, the inhaler technique device by device with the error that makes each one fail, the antidiabetics with their sick-day rules, the collyres (order, five-minute delay, the red eye that gets oriented), what is refused at the counter in self-medication, the antibiotics family by family (durée, prise, ce qui fait échouer), and what never stops abruptly — bêtabloquant, corticoïde, benzodiazépine, antidépresseur, opioïde, IPP, tabac, alcool. Three more answer what is asked every day and written down nowhere: the dermocorticoid classes with the unit-phalangette rule and the quantity each body part actually needs, the three levels of the driving pictogram — the risk of a hypnotic is not the night but the drive the next morning — and food against medicine, with a whole column for what is *not* true, because the counter spends as much time undoing beliefs as giving advice. The thirty-first is the one the biosimilars needed: what may be substituted at the counter and what may not, why the word « générique » is the wrong one to use, why a biological is traced by its brand *and* its lot number and never by its DCI alone, and why the injection technique is shown again at every change of brand — the pen is not the same pen. The last three answer what the season and the packaging do to a treatment: « Canicule », the ordonnance read against the heat — the diuretic and the IEC that empty a patient already losing water, the AINS that completes the triad, the lithium whose level rises with no change of dose, the anticholinergics that stop the only cooling the body has, the patch that delivers more when it is warm — with a column for what *not* to do, because most of the harm in a heatwave comes from a treatment stopped or doubled on somebody's own initiative; « Soleil », the photosensitisers told apart by mechanism, where doxycycline burns in hours while ketoprofen gel stays dangerous for two weeks after the last application; and « Pilulier », what must never be deconditioned — Pradaxa out of its blister, effervescents and orodispersibles that take up water, sublingual trinitrine whose active ingredient is volatile, and the « si besoin » dose that a weekly box silently turns into a schedule. The list runs down the left in five families, each row carrying its line count and a mark on the tables the team has corrected. Each table says when it was last read against its sources, on screen and on the printout, and one search field looks through all of them at once — every row that matches, wherever it lives, with the table it comes from.
- **« À programmer »** — the dashboard also carries the follow-up list: the accompaniments whose year is started, not finished, and with nothing in the agenda — how far each one is (« BPM 2/4 »), when the last entretien was, one click to the file. A sequence left half-done pays half.
- **« À revoir »** — the dashboard carries the call list: the files whose latest biology says something about their own treatments, and those whose ordonnance says something about itself, loudest first, the reason on hover, one click to the patient. It only appears when there is something on it.
- **Patient biology, read against the treatments** — a third tab on the patient file records what the laboratory gave (analyte, value, date), reads each result against its usual adult interval — normal, bas, élevé, or the critical threshold where it stops being a deviation — and draws the trend of any analyte with the bounds of its interval across it. Beside it, « Ce que ça change » applies eighty-seven rules that tie a value to what the file says the patient takes: a kaliémie above 5 under IEC or spironolactone, a DFG under 30 with an AOD or metformine, an INR above 5 under AVK, CPK at five times normal under a statine, a thrombopénie under héparine, an urée that rises before the créatinine does and catches a dehydration while suspending the diuretic is still enough, a hyperleucocytose under corticoïde that is the corticoïde and not an infection. A value alone informs; a value with the treatment behind it alerts. Forty-nine analytes, from the DFG to l'activité anti-Xa, each carrying the sentence that matters at the counter — and each carrying at least one rule, because an analyte without one is a number copied from the laboratory, which does it better.
- **La revue d'ordonnance** — what a set of treatments says about itself, read from the classes and tags the cards already carry: la triade néfaste (bloqueur du système rénine-angiotensine + diurétique + AINS), le double blocage, l'anticoagulant avec un AINS, la benzodiazépine avec un opioïde, l'anticholinergique prescrit sous anticholinestérasique, deux AINS, deux IPP, deux allongeurs du QT, la charge anticholinergique, trois sédatifs, la statine avec un fibrate, le clopidogrel avec l'oméprazole, le bêtabloquant caché dans un collyre du glaucome, la contraception sous inducteur enzymatique… Fifty-five rules, shown as chips on the patient file with the sentence on hover, and in full on the bilan. A fixed combination counts for both of its halves; a doublon needs two distinct boxes. And two of them fire on what is *absent*: an opioid with no laxative beside it, a corticothérapie with nothing for the bone — what is missing is half of what a bilan finds, and both rules go quiet the moment the missing line appears.
- **« Plan de prise »** — the patient's own copy of the entretien: one line per treatment (le médicament, à quoi ça sert, quand le prendre — the posology the file records for *this* patient, not the molecule's usual one, ce qu'il faut savoir — starting with what to do when a dose is missed), a box for their questions, and the pharmacy's number. The bilan stays at the officine; this one goes home.
- **La conciliation médicamenteuse** — a fifth tab on the patient file. Paste the hospital discharge prescription as it is written — one treatment per line, brand or DCI, the strength and the rhythm behind it — and the table says, treatment by treatment, what happened to it: reconduit, dose changée, arrêté, ajouté, or remplacé by a molecule of its own class. What *disappeared* is the point of the act: it is what the patient will keep taking if a box is left in the cupboard and nobody tells them. Stops and replacements come first, and a replacement is one row and not two — it is the divergence you go home with both boxes of. A line the base cannot match is never dropped in silence: it is shown exactly as it was typed, at the very top, because nobody has checked it. Matching goes through the file's own treatments before the 850 cards, so a line naming something the patient already takes means *that* treatment and not a homonym — otherwise a reconduction would read as a stop followed by an addition, a divergence invented out of nothing. One click writes the whole conciliation into the file's journal; another takes the discharge posologies onto the file; a third prints the sheet for the prescriber — the whole reconciliation, reconductions included, because a list of five changes says nothing about the twelve lines nobody looked at, with a blank box so the answer comes back on the same page.
...
- **A codex of preparations** — the officine's magistral and officinal formulas, fifty-eight to start with (vaseline salicylée, pâte à l'eau, crème à l'urée, gélules pédiatriques, sirop simple, glycérolé d'amidon, alcool à 70 %, gel hydroalcoolique de la formule OMS…), including the paediatric bench a child who cannot swallow a tablet depends on — captopril, furosémide and propranolol suspensions, each with the reason its shelf life is what it is — each with its formula, mode opératoire, conservation, what goes wrong, and its sources. Type the quantity actually being made and every line is rescaled — the excipient's « qsp 100 g » becomes « qsp 60 g » — with each ingredient's strength read off the formula beside it. The fiche de fabrication prints with a blank column for the lot of every raw material, the operator, the date and the boxes for the control and the labelling. Three calculators sit under the sheet: the titre, the dilution (C1·V1 = C2·V2) and a batch of capsules. Adding a preparation is adding a fiche.
- **Fiches for the dispositifs médicaux** — the officine dispenses at least as many dispositifs as medicines and had nowhere to write what it knows about them. Fifty-five fiches in eleven families: the dressings in the order you choose between them (hydrocolloïde, hydrocellulaire, alginate, hydrofibre, hydrogel, interface, argent, charbon, film, compresses), fixation, compression, stomie (colostomie, iléostomie, urostomie and the accessories that save the skin), sondage and incontinence, sets de soins, injection and diabetes, respiratoire, and the material the officine rents out — nébuliseur, TENS, lit médicalisé, matelas anti-escarre, fauteuil roulant, tire-lait. Each answers the six counter questions: for what, in which sizes, how it goes on, how often it is renewed, what the LPP line requires, and what goes wrong — the hydrocolloid's yellow gel that is taken for pus, the alginate that is never removed dry, the too-wide cut that reddens the skin around a stoma, the spacer left to air-dry without wiping. **The LPP field carries the rule, never the tarif**: a price shipped in a fiche is a price wrong within the year, and a test refuses the « € ». Prints as one A4 sheet or as a two-column booklet grouped by family. Like the codex, it is seeded once and the team's afterwards.
- **Rentals, counted and renewed** — a forfait de location runs on by itself and nobody re-reads it. A « Locations » tab on the patient file hands out a nébuliseur, a TENS, a lit médicalisé, takes it back, records a renewal — and counts the periods *entamées* against the forfait as it stood on the day it went out, never the tarif as it stands now. The dashboard raises a call list of the ordonnances that have lapsed or are about to, and the printed récapitulatif de facturation carries the rentals in their own table with their own total, never mixed into the acts'. **The forfaits ship empty**: `[locations]` in `config.toml`, edited in Options › Locations — the LPP moves, and a fee shipped in the application would be a fee wrong within the year. The arithmetic is `src/location.rs`, pure, tested and without an internal clock.
- **Insulin, as a shape** — « lente » on a box covers a glargine with no peak and an NPH that peaks at six hours, and it is that peak which makes the five-o'clock hypoglycaemia. Twelve insulins of the French market carry their action profile — onset, peak, duration — and on an insulin card that curve **replaces** the decay curve: the half-life of a subcutaneous depot answers a question nobody asks. In Calculs, the curves overlay on one axis so any two can be compared and shown to the patient. Under the drawing, the arithmetic a functional schema runs on: the 500 rule for carbohydrates, the 1800 rule for the sensitivity factor — in g/L *and* mmol/L, the two kept in step by a test, because that conversion is where a correction dose goes ten times wrong — the meal bolus, the correction, and the basal titration. `src/insulin.rs` is pure and tested; the rules read against the ordonnance, never in its place.
...
- **Two more answers on every card** — « En cas d'oubli » and « Ce qui doit faire consulter », the two questions asked most at the counter and written down least. Two hundred and thirty-two class rules fill them on 818 of the 850 starter cards, and only ever fill an empty field: a card the team has written to keeps what it says.
...
- **A printed mode d'emploi** — `F12` lists the shortcuts and prints the application's own handout: fifteen sections in two columns on one sheet, from unlocking the base to what the application deliberately does not decide. One copy beside the counter PC, one in the binder.
